...
const brandConfigs = {
    ...
    FLA: {
        ...
        // 🔵 Marino profundo (más luminoso que negro)
...
function ColeccionesMarcaPage() {
    ...
    }, [isBassari, isFlamenco, coleccionesNuevasBAS, coleccionesNuevasFLA, coleccionesBase]);
    ...
    // ✅ Ocultar CTA "descubre todas las colecciones" dentro del carrusel (si aparece)
    ...
    useEffect(() => {
        ...
        const toHide = Array.from(root.querySelectorAll("button")).filter((b) =>
            (b.textContent || "").toLowerCase().includes(
                (t("carousel.hideCtaMatch", "descubre todas las colecciones") || "").toLowerCase()
            )
        );
    ...
    // ✅ Slides por marca (tabs + progress + productos)
    // ✅ FIX texto: forzamos negro dentro de PresentacionColeccion para estética clara
            ...
            return [
                {
                  ...
                  .cjmw-presentacion-scope .text-black {
                    color: #111827 !important;
                  }
                `}</style>
                                <PresentacionColeccion
                                    titulo={t("newCollections.bassari.essences.title")}
                                    imagenFondo={imagenes["ESSENCES DU NIL"] || coversOverride.BAS["ESSENCES DU NIL"]}
                                    descripcion={t("newCollections.bassari.essences.description")}
                                />
                            </div>
                ...
                `}</style>
                                <PresentacionColeccion
                                    titulo={t("newCollections.bassari.voyage.title")}
                                    imagenFondo={
                                        imagenes["LE VOYAGE DES WOLOF"] || coversOverride.BAS["LE VOYAGE DES WOLOF"]
                                    }
        ...
        return [
            {
              ...
              `}</style>
                            <PresentacionColeccion
                                ...
                                imagenFondo={imagenes["INDIENNE STRIPES"] || coversOverride.FLA["INDIENNE STRIPES"]}
    ...
    }, [isNuevaColeccionBrand, isBassari, imagenes, coversOverride, t]);

    // ✅ Hero: primera colección con imagen disponible (override > API)
    ...
    return (
        <CartProvider>
            ...
            {/* ✅ Estética luminosa: fondo blanco + texto negro */}
            <main className="bg-white text-gray-900">
                {/* ===================== HERO ===================== */}
                <section className={`relative overflow-hidden bg-gradient-to-br ${config.gradient}`}>
                    {/* overlay suave para que el gradiente no sea agresivo */}
                    ...
                    {isNuevaColeccionBrand && heroImage && (
                        <div className="absolute inset-0 opacity-20">
                            <img src={heroImage} alt={t("hero.imageAlt")} className="h-full w-full object-cover" />
                            {/* overlay claro para mantener luminosidad */}
                    ...
                    <div className="relative max-w-6xl mx-auto px-6 py-16 lg:py-20">
                        <Reveal>
                            <p className={`uppercase tracking-[0.4em] text-xs ${config.accent}`}>
                                {t(isNuevaColeccionBrand ? "hero.label.newCollections" : "hero.label.collections")} {brandName}
                            </p>
                        </Reveal>
                        ...
                        <Reveal delay={360}>
                            <div className="mt-10 flex flex-wrap gap-4">
                                ...
                                <button
                                    onClick={() => navigate(`/${marcaKey === "BAS" ? "bassariHome" : "flamencoHome"}`)}
                ...
                {/* ===================== NUEVAS COLECCIONES + PRODUCTOS (BAS/FLA) ===================== */}
                                ...
                                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                                    <div>
                                        ...
                                        <h2 className="mt-4 text-3xl md:text-4xl font-semibold text-gray-900">
                                            {t(isBassari ? "sections.featured.title.bassari" : "sections.featured.title.flamenco")}
                                        </h2>
                                    </div>
                                    <div className="text-gray-600 max-w-xl">{t("sections.featured.subtitle")}</div>
                                </div>
                            </Reveal>

                            <div
                                ref={carouselWrapperRef}
                                className="mt-10 rounded-[2.25rem] border border-gray-200 bg-white shadow-[0_20px_60px_rgba(0,0,0,0.08)] overflow-hidden"
                            >
                                <CarruselColeccionesNuevas slides={slidesNuevas} durationMs={15000} stickyHeader />
                            </div>
                    ...
                    <div className="max-w-6xl mx-auto px-6 grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
                        <Reveal>
                            <div className="space-y-6">
                                ...
                                <p className="text-gray-700 text-lg leading-relaxed">{t(config.highlightKey)}</p>
                        ...
                        <Reveal delay={180}>
                            <div className={`rounded-3xl p-8 ${config.panel} backdrop-blur-lg shadow-xl border border-gray-200`}>
                                ...
                                <p className="mt-4 text-gray-700">{t("sections.story.panel.description")}</p>
                                <button
                                    onClick={() => navigate(buildProductUrl())}
                        ...
                        <Reveal>
                            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                                <div>
                                    ...
                                    <h2 className="mt-4 text-3xl md:text-4xl font-semibold text-gray-900">
                                        {t(isNuevaColeccionBrand ? "sections.grid.title.new" : "sections.grid.title.all")}
                                    </h2>
                                </div>

                                {/* ✅ Quitamos el botón final de venta/asesoría (CTA final eliminada),
                    pero mantenemos este "Ver todo" porque navega al catálogo */}
                                <button
                                    onClick={() => navigate(buildProductUrl())}
                        ...
                        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {colecciones.map((coleccion, index) => (
                                <Reveal key={coleccion} delay={index * 80}>
                                    <button
                                        onClick={() => navigate(buildProductUrl({ collection: coleccion }))}
                                    ...
                                    >
                                        <div className="relative overflow-hidden rounded-3xl shadow-[0_18px_50px_rgba(0,0,0,0.10)] border border-gray-200 bg-white">
                                            ...
                                            <div className="absolute inset-x-6 bottom-6">
                                                ...
                                                <h3 className="mt-2 text-xl font-semibold text-gray-900">{coleccion}</h3>
                                            </div>
                                        </div>
                ...
                {/* ✅ ELIMINADO: CTA FINAL (venta/asesoría) */}
            </main>
