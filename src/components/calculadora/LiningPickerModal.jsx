...
import { searchLinings, getProductColors } from '../../api/products';
import { defaultImageUrlModalProductos } from '../../Constants/constants';

const Q_BUENA = 'PRODUCTO_BUENA';
const Q_BAJA = 'PRODUCTO_BAJA';

export default function LiningPickerModal({ open, onClose, onPicked, nameCandidates = [] }) {
    ...
    // STEP 0 → cargar productos (forros)
    useEffect(() => {
        ...
        const handler = setTimeout(async () => {
            try {
                const baseItems = await searchLinings({ names: nameCandidates, q });
                ...
                if (!alive) return;
                setItems([]);
                setError(e?.message || 'No se pudieron cargar los forros');
            } finally {
    ...
    }, [open, step, q, nameCandidates, getThumbForId]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                console.log('COLORES RAW DESDE BACKEND:', rawColors);
                ...
                const enriched = (rawColors || []).map((c) => ({
                    ...
                    // La URL viene completa del backend: usar tal cual
                    imageUrl: c.ficadjunto || c.ficAdjunto || defaultImageUrlModalProductos,
                }));
            ...
            <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-[36rem] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl">
                <div className="flex items-center justify-between border-b px-4 py-3">
                    <h4 className="text-sm font-semibold">
                        {step === 0 ? 'Selecciona el forro' : `Colores de: ${selectedProduct?.name}`}
                    </h4>
