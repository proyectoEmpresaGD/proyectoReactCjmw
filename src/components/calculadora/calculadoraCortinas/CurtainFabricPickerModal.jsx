// src/components/calculadora/CurtainFabricPickerModal.jsx
import { useEffect, useState } from 'react';
import { createPortal } from 'react-dom';
import { searchCurtainFabrics, getProductColors } from '../../../api/products';
import { cdnUrl } from '../../../Constants/cdn';
import { defaultImageUrlModalProductos } from '../../../Constants/constants';

export default function CurtainFabricPickerModal({ open, onClose, onPicked }) {
    ...
    // STEP 0 → buscar tejidos con uso CORTINA
    useEffect(() => {
        ...
        const handler = setTimeout(async () => {
            try {
                const baseItems = await searchCurtainFabrics({ q }); // [{id,name,pricePerMeter, imageUrl?, ancho?}]
                // enriquecer con miniatura (Buena/Baja) usando tu CDN helper
                const enriched = await Promise.all(
                    (baseItems || []).map(async (p) => {
                        try {
                            // si ya trae imageUrl lo mantenemos
                ...
                if (!alive) return;
                setItems([]);
                setError(e.message || 'No se pudieron cargar los tejidos de cortina');
            } finally {
            ...
            <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-[36rem] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl">
                ...
                <div className="flex items-center justify-between border-b px-4 py-3">
                    <h4 className="text-sm font-semibold">
                        {step === 0 ? 'Elige tu tejido (uso CORTINA)' : `Colores de: ${selectedProduct?.name}`}
                    </h4>
                                    ...
                                    <li key={p.id} className="flex items-center gap-3 p-3 hover:bg-gray-50">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="text-xs text-gray-500">
                                                {typeof p.pricePerMeter === 'number' ? `${p.pricePerMeter.toFixed(2)} €/m` : '—'}
                                                {p.ancho ? ` · ${p.ancho}` : ''}
                                            </div>
                                        </div>
                                        <button
                            ...
                            <div className="grid grid-cols-2 gap-3 max-h-80 overflow-auto pr-1">
                                {colors.map((c) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            onPicked?.({
                                                product: selectedProduct,                      // incluye pricePerMeter, ancho si viene
                                                color: c,
