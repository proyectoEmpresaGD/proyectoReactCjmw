...
function BloquesInformativos({ titulo, texto, bloques = [] }) {
    ...
    // Tailwind: clases de delay predefinidas
    ...
    return (
        <section className="flex flex-col items-center text-center px-4 py-10">
            ...
            <div
                className={
                    // Móvil/Tablet: se mantiene igual (2 cols). Escritorio: grid fluido que ocupa todo el ancho.
                    // Nota: la clase arbitraria de Tailwind usa JIT: [grid-template-columns:...]
                    'grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 gap-6 w-full max-w-7xl ' +
                    'lg:[grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))]'
                }
            >
                {bloques.map((bloque, idx) => {
                    const delayClass = delayClasses[idx % delayClasses.length];
                    const isVisible = bloquesVisibles.includes(idx);
                    return (
                        <div
                            key={idx}
                            data-index={idx}
                            ...
                            className={`group relative h-64 sm:h-72 overflow-hidden rounded-md shadow-md hover:cursor-pointer transition-all duration-700 ease-out ${isVisible ? `opacity-100 translate-y-0 ${delayClass}` : 'opacity-0 translate-y-6'
                                }`}
                        >
                            <div
