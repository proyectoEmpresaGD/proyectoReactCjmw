...
const contactValidation = [
    ...
    body('firstName')
        ...
        .matches(/^[\p{L}\s.'-]+$/u)
        .withMessage('invalid_first_name_format'),
    ...
    body('lastName')
        ...
        .matches(/^[\p{L}\s.'-]+$/u)
        .withMessage('invalid_last_name_format'),
    ...
    body('province')
        ...
        .isLength({ max: 120 })
        .withMessage('invalid_province'),

    // Empresa obligatoria si perfil profesional
    body('company')
        .optional({ checkFalsy: true })
        .isLength({ max: 160 })
        .withMessage('invalid_company'),
    ...
    body('marketing')
        .optional()
        .isIn(['true', 'false'])
        .withMessage('invalid_marketing'),

    body('consent')
        .equals('true')
        .withMessage('invalid_consent'),

    body('consentVersion')
        .optional()
        .isLength({ max: 50 })
        .withMessage('invalid_consent_version'),
];
...
export const createContactRouter = ({ pool, contactModel }) => {
    ...
    if (!modelInstance) {
        throw new Error('A contact model or PostgreSQL pool is required to initialise the contact router.');
    }

    const router = Router();
    const transporter = buildMailer();
    const contactInbox = process.env.CONTACT_INBOX || process.env.PEDIDOS_EMAIL;
    const controller = new ContactController({
        contactModel: modelInstance,
        transporter,
        contactInbox
    });
        ...
        upload.single('attachment')(req, res, (err) => {
            ...
            if (err.message === 'INVALID_FILE_TYPE') {
                return res.status(400).json({
                    error: 'Unsupported file type. Allowed formats: PDF, JPG, PNG.'
                });
            }

            if (err.code === 'LIMIT_FILE_SIZE') {
                return res.status(400).json({
                    error: 'File exceeds the maximum allowed size of 2 MB.'
                });
            }
