...
// Función para convertir valores a enteros seguros
function safeParseInt(value, defaultValue = null) {
    const parsed = Number(value);
    return Number.isNaN(parsed) ? defaultValue : parseInt(parsed);
}

const importDataClientes = async () => {
    const  = await pool.connect();

    try {
        // Asegúrate de que el archivo 'clientes.json' se encuentre en el mismo directorio que este script
        ...
        for (const item of data) {
            ...
            const params = [
                ...
                item.CodRepre, safeParseInt(item.Comision), item.NRB, item.Asegurado, item.IDCP, item.CodRiesgo, safeParseInt(item.ImpRiesgo),
                item.Portes, item.tipPortes, item.DadoBaja, item.CodCtaContab, item.CodIva, item.ForEnvio, item.ComManual,
                item.Bloqueado, safeParseInt(item.FecAlta), item.PermitirAlbSinPedido, item.ReqConfirParTrabajo, item.CodTipPersona,
                item.ImpPreNetos, item.TrabajaConRE, item.CodProvi, item.ExcluirBloqDocCobrosVencidosPendientes
            ];
